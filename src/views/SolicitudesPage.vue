...
import { uploadTransferVoucher, updateTransferState } from '@/services/transferService';
import { getTransferUnitsFiltered } from '@/services/unitService';
...
async function handleVoucherSubmit(file) {
  ...
  try {
    const response = await uploadTransferVoucher(selectedTransferId.value, file);
...
async function confirmApprove() {
  ...
  try {
    const response = await updateTransferState(selectedTransferId.value, 2); // 2 = Approved
