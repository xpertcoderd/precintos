<template>
  <div class="p-4 md:p-8 bg-slate-50 min-h-full">
    ...
    <main class="bg-white rounded-2xl shadow-lg p-6">
      ...
      <!-- Modals -->
    <EntityModal :visible="isModalVisible" :title="modalTitle" :save-text="saveButtonText" @close="closeModal" @save="handleSave" custom-layout max-width="max-w-5xl">
      <UsuarioForm 
        ref="usuarioFormRef" 
...
async function handleSave() {
  // This function is no longer used directly by the modal's save button in custom layout,
  // but kept for compatibility if needed. The form now emits 'save' directly to saveItem.
}
</script>
