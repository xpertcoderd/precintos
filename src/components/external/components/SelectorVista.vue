<template>
	<div class="contendor">
		<div class="selectorVista">
			...
			<div @click="fuction_cuatro" class="targetas text-center" :class="{ 'vistaSelected': vista.cuatro }">
				{{ labels[3] }}
			</div>
...
const labels = ref(['Uno', 'dos', 'tres', 'cuatro'])

const incomingData = defineProps(['labels'])

const outGoingData = defineEmits(['fuction_uno', 'fuction_dos', 'fuction_tres', 'fuction_cuatro'])
...
function fuction_cuatro() {
	resetBtn()
	vista.value.cuatro = true
	outGoingData('fuction_cuatro');
}


function updateLabels(label) {

	labels.value = label || ["Contenedor", "BL", "ETA", "Mapa"]
}
