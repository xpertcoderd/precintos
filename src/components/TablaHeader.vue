	...
	<div class="rowMenu"> <!-- columnaName -->
		...
		<div>
			...
			<button @click="outGoingData('openLock')" v-if="incomingData.btn" type="button"
				:class="{ 'disabled': !incomingData.btnEnable, 'btn-secondary': !incomingData.btnEnable }"
				class="btn btn-primary mybtn">Abrir Candado</button>
			<!-- :class="{ 'disabled': !showMapaBtn, 'btn-secondary': !showMapaBtn }" -->
			<span v-if="incomingData.btn">
				<button v-if="mapabtn" @click="mostrarMapa" type="button" class="btn btn-primary mybtn">Mostrar En
					Mapa</button>

				<button v-if="!mapabtn" @click="mostrarLista" type="button"
					class="btn btn-primary mybtn">Regresar</button>
...
const incomingData = defineProps(['btn', 'btnEnable', 'filtro']); //'showMapaBtn',  'ismapa'
const outGoingData = defineEmits(['openLock', 'buscar', 'buscarisEmpty', 'searchMode', 'showInMapa', 'showList']); //''

const mapabtn = ref(true)
...
function mostrarMapa() {
	outGoingData('showInMapa')
	mapabtn.value = false
}

function mostrarLista() {
	outGoingData('showList')
	mapabtn.value = true
}
